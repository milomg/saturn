...
fn write_region_contents(destination: &Path, contents: &str) {
    let Ok(value) = base64::engine::general_purpose::STANDARD.decode(contents) else {
        return;
    };
...
#[tauri::command]
pub async fn export_binary_contents(
    data: Vec<u8>,
    filters: Vec<AccessFilter>,
    state: tauri::State<'_, AccessManager>,
) -> Result<String, ()> {
    let destination = state
        .select_save("Save File", &filters, false)
        .await
        .ok_or(())?;
...
#[tauri::command]
pub async fn export_hex_contents(
    data: &str,
    state: tauri::State<'_, AccessManager>,
) -> Result<String, ()> {
    let destination = state
        .select_save("Save Region File", &[], false)
        .await
        .ok_or(())?;
...
#[tauri::command]
pub async fn export_hex_regions(
    regions: Vec<HexRegion>,
    state: tauri::State<'_, AccessManager>,
) -> Result<String, ()> {
    let destination = state
        .select_save("Save Directory", &[], false)
        .await
        .ok_or(())?;
