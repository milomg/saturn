...
export function mergeResults(
  ...input: FuseResult<Suggestion>[][]
): FuseResult<Suggestion>[] {
  ...
  while (true) {
    ...
    for (const value of values) {
      ...
      if (
        !next ||
        (next[next.length - 1].score ?? 0) >
          (value[value.length - 1].score ?? 0)
      ) {
        next = value
      }
    }
...
const instructionParts = [
  ...
  { name: 'Set < Immediate', replace: 'slti' },
  { name: 'Set < Immediate Unsigned', replace: 'sltiu' },
  ...
  { name: 'Branch < Unsigned', replace: 'bltu' },
  { name: 'Branch > Unsigned', replace: 'bgtu' },
  { name: 'Branch <= Unsigned', replace: 'bleu' },
  { name: 'Branch >= Unsigned', replace: 'bgeu' },
