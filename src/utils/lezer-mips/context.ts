...
import { MarkedSuggestion } from '../languages/suggestions'

interface InsightUpdate {
  startLine: number
  count: number
  updates: MarkedSuggestion[][]
}
...
interface SuggestionContext {
  lines: MarkedSuggestion[][]
  all: MarkedSuggestion[]
}

export const suggestions = StateField.define<SuggestionContext | null>({
  create() {
    // Assumption: Switching tabs will also create new StateFields (it almost definitely does).
    return null
  },
  update(value, tr) {
    let lines: MarkedSuggestion[][]
